...
export const getServerSideProps = async (context) => {
    ...
    const vergleichDataUrl = `/stimmkreisvergleichvorjahr_nonmat?order=prozent2018.desc&stimmkreis=eq.${context.params.stimmkreis}`;
    const vergleichData = await loadData(vergleichDataUrl);


    const detailDataUrl = `/stimmkreisdetails_nonmat?stimmkreis=eq.${context.params.stimmkreis}&landtagswahl=eq.2018`;
...
export default function Stimmkreis({preVergleichData, preDetailData}) {
    ...
    useEffect(() => {
        ...
        loadData(`/stimmkreisvergleichvorjahr_nonmat?order=prozent2018.desc&stimmkreis=eq.${stimmkreis}`, setVergleichData);
    }, [stimmkreis]);

    useEffect(() => {
        if (stimmkreis == null) return;

        loadData(`/stimmkreisdetails_nonmat?stimmkreis=eq.${stimmkreis}&landtagswahl=eq.2018`, o => setDetailData(o[0]));
    }, [stimmkreis]);
